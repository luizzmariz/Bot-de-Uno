...
class Uno:
    ...
    def game(self):
        while(True):
            ...
            if card_played.value in UnoCard.numbers[:10]:
                print(f"{player.name} played a action card: {card_played}.")
                match card_played.value:
                    case 'Skip':
                        next_player = self.next_player()
                        print("Player " + next_player + " has lost its turn due to Skip.")
                    case 'Reverse':
                        self.direction *= -1
                        print("Game direction has changed to " + self.direction + " due to Reverse")
                    case 'Draw Two':
            ...
            elif card_played.value in UnoCard.wild_cards:
